...
class Annotator:
    ...
    def box_label(self,anon, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if anon:
            ...
            box_size = w1*h1
            kernel = []
            if box_size < 1336:  #0.5% of img size
                kernel = [7,7]
            elif box_size < 4608: #1.5% of img size
                kernel = [15,15]
            elif box_size < 7680: #2.5% of img size
                kernel = [20,20]
            elif box_size < 15360: #5% of img size
                kernel = [25,25]
            else:
                kernel = [40,40]
            ROI = self.im[y1:y1 + h1,x1:x1 + w1]
            row, col, ch = ROI.shape
            gauss = np.random.uniform(0.70, 1.3, (row, col, ch)).reshape(row, col, ch)
            blur = cv2.blur(ROI, (kernel[0],kernel[1]))
            blur_noise = np.clip(((blur * gauss)),25,230)
         
            self.im[y1:y1 + h1,x1:x1 + w1] = blur_noise

        else:
            cv2.rectangle(self.im, p1, p2, color, thickness=int(self.lw/2), lineType=cv2.LINE_AA)
    ...
    def rectangle(self, xy, fill=1, outline=None, width=1):
